...
###   This script attempts to download the signature file SHA256SUMS.asc from islambit.org
...
WORKINGDIR="/tmp/islambit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://islambit.org/bin/"
VERSIONPREFIX="islambit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'islambit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Islambit Core binary release signing key installed?"
   fi
